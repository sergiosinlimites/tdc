...
% Paso 5: guardar resultados completos y exportar figuras con fondo blanco.
...
fprintf('SAS/CAS theta estable: %d | SAS/CAS phi estable: %d\n', ...
...
for k = 1:numel(sim_results.summary)
    ...
    fprintf('%-4s %-18s RMS theta=%7.3f deg RMS phi=%7.3f deg sat=%5.1f%%\n', ...
        display_controller(row.controller), row.scenario, row.theta_rms_error_deg, ...
...
function name = display_controller(name)
if strcmp(name, 'pid')
    name = 'sas';
end
end
