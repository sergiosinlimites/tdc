...
% W3_phi_high, W3_phi_cross. Los primeros candidatos conservan el barrido
% de tracking; los ultimos atacan rechazo de ruido/perturbacion lateral.
base = [ ...
    80 1.00 1.00 8 15 cfg.spec.wp; ...
    100 0.80 1.80 cfg.spec.wp 15 cfg.spec.wp; ...
    120 0.80 2.00 cfg.spec.wp 15 cfg.spec.wp; ...
    120 0.60 2.20 cfg.spec.wp 15 cfg.spec.wp; ...
    140 0.70 2.40 cfg.spec.wp 15 cfg.spec.wp; ...
    160 0.80 2.50 cfg.spec.wp 15 cfg.spec.wp; ...
    180 0.70 2.80 cfg.spec.wp 15 cfg.spec.wp; ...
    200 0.60 3.00 cfg.spec.wp 15 cfg.spec.wp; ...
    160 1.00 3.00 cfg.spec.wp 15 cfg.spec.wp; ...
    220 0.80 3.20 cfg.spec.wp 15 cfg.spec.wp; ...
    260 1.00 4.50 20 35 25; ...
    300 1.00 5.00 18 40 22; ...
    320 1.00 5.50 18 45 22];
...
for k = 1:size(base, 1)
    ...
    grid(k).W3_phi_high_gain = base(k, 5); %#ok<AGROW>
    grid(k).W3_phi_cross_frequency = base(k, 6); %#ok<AGROW>
end
end
...
row.W3_phi_high_gain = candidate.W3_phi_high_gain;
row.W3_phi_cross_frequency = candidate.W3_phi_cross_frequency;
...
row = struct('candidate', NaN, 'W1_phi_low_gain', NaN, ...
    ...
    'W2_phi_cross_frequency', NaN, 'W3_phi_high_gain', NaN, ...
    'W3_phi_cross_frequency', NaN, 'gamma_theta', NaN, 'gamma_phi', NaN, ...
...
fprintf(fid, '| cand | W1 phi low | W2 phi low | W2 phi high | W2 cross | W3 phi high | W3 cross | gamma theta | gamma phi | KS theta | KS phi | RMS theta | RMS phi | err phi30 | err phi40 | err noise | sat max %% | score | ok |\n');
fprintf(fid, '|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|:---:|\n');
