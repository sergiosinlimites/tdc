%% Construye taller1_uav.slx desde cero
% Modelo Simulink lineal acoplado con selector PID/H_inf, ruido de medicion
% y saturacion de actuadores.

clear; clc;

% Paso 1: cargar variables numericas que usaran los bloques.
project_dir = fileparts(mfilename('fullpath'));
addpath(project_dir);
run(fullfile(project_dir, 'init_taller1_simulink.m'));

% Paso 2: cerrar y borrar una version anterior para regenerar el modelo.
mdl = 'taller1_uav';
...
% Paso 3: crear el sistema y configurar solver/tiempo de simulacion.
new_system(mdl);
open_system(mdl);
...
% Paso 4: agregar referencias, ruido y selector de modo de control.
...
% Paso 5: agregar la planta lineal acoplada y separar sus salidas medidas.
...
% Paso 6: construir medicion ruidosa y error de theta.
...
% Paso 7: construir medicion ruidosa y error de phi.
...
% Paso 8: agregar PID, H_inf, selector, saturacion y perturbacion de elevator.
add_block('simulink/Continuous/Transfer Fcn', [mdl '/PID_theta'], ...
    'Numerator', 'pid_theta_num', 'Denominator', 'pid_theta_den', ...
...
% Paso 9: agregar PID, H_inf, selector, saturacion y perturbacion de aileron.
add_block('simulink/Continuous/Transfer Fcn', [mdl '/PID_phi'], ...
    'Numerator', 'pid_phi_num', 'Denominator', 'pid_phi_den', ...
...
% Paso 10: agregar amortiguador de yaw comun y perturbacion de rudder.
...
% Paso 11: reunir las 8 entradas del linmodel en el orden esperado.
add_block('simulink/Signal Routing/Mux', [mdl '/u_mux'], ...
    'Inputs', '8', 'Position', [760 185 775 315]);

% Paso 12: agregar logging de salidas principales y controles.
...
% Paso 13: conectar planta y separar salidas medidas.
add_line(mdl, 'UAV_linmodel/1', 'y_demux/1', 'autorouting', 'on');
add_line(mdl, 'u_mux/1', 'UAV_linmodel/1', 'autorouting', 'on');

% Paso 14: conectar lazo theta. Salidas de y_demux:
% 1 theta, 2 phi, 3 p, 4 q, 5 r.
...
add_line(mdl, 'theta_error/1', 'PID_theta/1', 'autorouting', 'on');
...
add_line(mdl, 'PID_theta/1', 'switch_theta/3', 'autorouting', 'on');
...
add_line(mdl, 'phi_error/1', 'PID_phi/1', 'autorouting', 'on');
...
add_line(mdl, 'PID_phi/1', 'switch_phi/3', 'autorouting', 'on');
...
% Paso 17: mapear comandos a las entradas de planta.
% Mux input order: throttle, elevator, rudder, l_aileron, r_aileron,
...
% Paso 18: conectar senales de salida y control al workspace.
...
% Paso 19: guardar el modelo ya cableado.
